...
chdir("C:\\Users\\annal\\Documents\\GitHub\\affixproject\\data\\BASLv3\\WITHOUTmorphspelling") # set working directory to responses folder
allfiles = [f for f in listdir() if isfile(f)] # get all file names

all_data = []
column_names = ['rt','response','trial_type','time_elapsed','internal_node_id','subject_id','study_id','session_id','stimulus','question_order','success','task','item','correct_response','testing_condition','correct','target','confound','name','view_history']
for file in allfiles:
    ...
    if len(temp[0]) == 19:
        column_names = ['rt','response','trial_type','time_elapsed','internal_node_id','subject_id','study_id','session_id','stimulus','question_order','success','task','item','correct_response','testing_condition','correct','target','confound','name']
...
### EXPORTING ###
all_familiarity_data_scored.to_csv("C:\\Users\\annal\\Documents\\GitHub\\affixproject\\exp3_familiarity_preprocessed.csv", index=True, header=True)
all_testing_data_scored.to_csv("C:\\Users\\annal\\Documents\\GitHub\\affixproject\\exp3_testing_preprocessed.csv", index=True, header=True)
all_BLP_data.to_csv("C:\\Users\\annal\\Documents\\GitHub\\affixproject\\exp3_BLP_preprocessed.csv", index=True, header=True)