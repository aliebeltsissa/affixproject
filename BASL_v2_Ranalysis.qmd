...
ggsave("exp2_yes_allconditions_scatterplot.png",width=10,height=7,yes_all_plt,path=output_folder,device="png");
...
# More balanced multi = more sig morph fam effect & 1M "yes" more different from 0M "yes"
```
...
t.test(BLP_2Mscores$score_2M[BLP_2Mscores$script=='mixed/diff'],mu=0.5);
```
...
```{r}
script_short <- select(data_testing_2M_script_aggr[data_testing_2M_script_aggr$response=="meanYes",],c("sbj_ID","expected","script","meanValue"));

script_averages <- data.frame(
  expected=rep(c("between_lang","within_lang"),2),
  script=c(rep("same",2),rep("mixed",2)),
  mean=c(mean(script_short[script_short$script=="same" & script_short$expected=="between_lang",]$meanValue),mean(script_short[script_short$script=="same" & script_short$expected=="within_lang",]$meanValue),mean(script_short[script_short$script=="mixed/diff" & script_short$expected=="between_lang",]$meanValue),mean(script_short[script_short$script=="mixed/diff" & script_short$expected=="within_lang",]$meanValue)),
  se=c(sd(script_short[script_short$script=="same" & script_short$expected=="between_lang",]$meanValue)/sqrt(length(script_short[script_short$script=="same",]$script)/2),sd(script_short[script_short$script=="same" & script_short$expected=="within_lang",]$meanValue)/sqrt(length(script_short[script_short$script=="same",]$script)/2),sd(script_short[script_short$script=="mixed/diff" & script_short$expected=="between_lang",]$meanValue)/sqrt(length(script_short[script_short$script=="mixed/diff",]$script)/2),sd(script_short[script_short$script=="mixed/diff" & script_short$expected=="within_lang",]$meanValue)/sqrt(length(script_short[script_short$script=="mixed/diff",]$script)/2))
);

script_averages$upper <- script_averages$mean+script_averages$se/2;
script_averages$lower <- script_averages$mean-script_averages$se/2;

script2M_plt <- ggplot(data_testing_2M_script_aggr[data_testing_2M_script_aggr$response=="meanYes",], aes(x=script, y=meanValue, fill=expected)) +
  geom_hline(yintercept=0.5, linetype="dashed", color="darkgrey",lwd=1.25) +
  geom_violin(alpha=0.75,width=1) +
  
  geom_errorbar(aes(x=0.75,ymin=script_averages[script_averages$script=="same" & script_averages$expected=="between_lang",]$lower,ymax=script_averages[script_averages$script=="same" & script_averages$expected=="between_lang",]$upper),width=.1) + # same script, between-language
  annotate("point",x=0.75,y=script_averages[script_averages$script=="same"&script_averages$expected=="between_lang",]$mean,colour="darkred",size=4) + # same script, between-language
  
  geom_errorbar(aes(x=1.25,ymin=script_averages[script_averages$script=="same" & script_averages$expected=="within_lang",]$lower,ymax=script_averages[script_averages$script=="same" & script_averages$expected=="within_lang",]$upper),width=.1) + # same script, within-language
  annotate("point",x=1.25,y=script_averages[script_averages$script=="same"&script_averages$expected=="within_lang",]$mean,colour="lightblue",size=4) + # same script, within-language
  
  geom_errorbar(aes(x=1.75,ymin=script_averages[script_averages$script=="mixed" & script_averages$expected=="between_lang",]$lower,ymax=script_averages[script_averages$script=="mixed" & script_averages$expected=="between_lang",]$upper),width=.1) + # mixed script, between-language
  annotate("point",x=1.75,y=script_averages[script_averages$script=="mixed"&script_averages$expected=="between_lang",]$mean,colour="darkred",size=4) + # mixed script, between-language
  
  geom_errorbar(aes(x=2.25,ymin=script_averages[script_averages$script=="mixed" & script_averages$expected=="within_lang",]$lower,ymax=script_averages[script_averages$script=="mixed" & script_averages$expected=="within_lang",]$upper),width=.1) + # mixed script, within-language
  annotate("point",x=2.25,y=script_averages[script_averages$script=="mixed"&script_averages$expected=="within_lang",]$mean,colour="lightblue",size=4) + # mixed script, within-language
  
  #geom_boxplot(position=position_dodge(width=1),width=0.25) +
  scale_x_discrete(name="Script",labels=c('Same','Mixed')) +
  scale_fill_manual(name="Expected",labels=c('Between-language', 'Within-language'),values=c(cols2[100],cols2[200])) +
  labs(x = "String nature", y = 'Prop of "within-language" responses') +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"),
        axis.text = element_text(family = "CMU Serif", size = 30, color = "black"),
        text=element_text(family="CMU Serif",size=30),legend.position="inside",
        legend.position.inside=c(0.8,0.1),legend.background=element_rect(fill=NA));
```
