...
#t=2.04 p=0.04 CI=[50.08;54.60] -> 0M "yes" sig above chance
...
#t=-2.00 p=0.049 CI=[0.45;0.50] -> 0M scores sig below chance
...
#t=8.2 p=3.46e-14 CI=[56.11;60.00] -> sig above chance
...
#t=8.22 p=3.46e-14 CI=[0.56;0.60] -> sig above chance
...
#t=12.5 p<2.2-16 CI=[61.03;65.16] -> sig above chance
...
par(mar=c(2,5,2,2));
boxplot(data_testing_2M_means$score_2M, ylab = "2M accuracy score",width=2,
        family="Montserrat",ylim=c(0,1),cex.lab=2,cex.axis=1.75,yaxs="i");
abline(h=0.5, lty=5);
...
#t=-0.83 p=0.41 CI=[0.48;0.51] df=183 -> not sig different from chance
...
cor.text(data_testing$dprime,data_testing$c, method="pearson"); # r = 0.03
...
ggplot(data_testing_conditions, aes(x = condition, y = score, color = condition)) +
  geom_hline(yintercept=0.5, linetype="dashed", 
             color = "darkgray",lwd=1.25) +
  geom_jitter(width = 0.2, height = 0, alpha = 0.3,color= "black",size=2) +
...
# 2M accuracy*RTs
cor(data_testing_2M_means$score_2M, data_testing_rt_means$x); # r = 0.09
# very slightly, longer RT = better accuracy
...
#t=-4.56 p=5.39e-6 CI=[0.491;0.496] -> sig different from chance
...
#t=-19 p<2.2e-16 CI=[0.484;0.487] -> sig different from chance
...
boxplot(data_familiarity_means$x, ylab = "Familiarity score",ylim=c(0,1),
        width=2,cex.lab=2,cex.axis=1.75,yaxs="i");
abline(h=0.5, lty=5);
par(mar=c(5, 4, 4, 2) + 0.1); # back to default
...
# t=10.2, p<2.2e-16, CI = [0.56;0.59] df=183 -> sig above chance
...
cor(data_familiarity_means$x, data_testing_2M_means$score_2M); # r = 0.02
...
#t=4.87 p=3.10e-5 CI=[0.56;0.64] df=31 -> sig above chance
...
#t=6.44 p=9e-9 CI=[0.55;0.59] df=78 -> sig above chance
...
# n=156
same_mixed_script_sbjID <- c(same_script_sbjID,mixed_script_sbjID);
# n=174
diff_mixed_script_sbjID <- c(diff_script_sbjID,mixed_script_sbjID);
# n=28
...
ggplot(data_BLP, aes(x=sbj_ID,y=ent,colour=category)) +
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"),
        axis.text = element_text(family = "Montserrat", size = 20, color = "black"),
        axis.text.x=element_blank(),
        text=element_text(family="Montserrat",size=20),
        legend.position=c(0.15,0.25),legend.background=element_rect(fill=NA)) +
  scale_color_manual(values=c(cols3[1],cols3[3],cols3[5],cols3[9]),
                     name="Category",
...
#ent & multiexp
par(mar=c(5,5,2,2));
plot(data_BLP$ent,data_BLP$multiexp,pch=19,
     ylab="Multilingual experience",xlab="Entropy",
     cex.lab=1.5);
cor.test(data_BLP$ent,data_BLP$multiexp,method="pearson");
# t = 30.175, df = 182, p-value < 2.2e-16, CI=[0.89;0.93], est=0.91 (***)
text(0.25,600,"r = 0.91",cex=1.5); 

#cossim & multiexp
plot(data_BLP$cossim,data_BLP$multiexp,pch=19,
     ylab="Multilingual experience",xlab="Cosine similarity",
     cex.lab=1.5);
cor.test(data_BLP$cossim,data_BLP$multiexp,method="pearson");
# t = -1.617, df = 182, p-value = 0.1076, CI=[-0.26;0.03], est=-0.12 ()
text(0.85,650,"r = -0.12",cex=1.5);

#cossim & ent
cor.test(data_BLP$cossim,data_BLP$ent,method="pearson");
#t=-4.85 df=182 p=2.63e-6 CI=[-0.46;-0.20]
plot(data_BLP$cossim,data_BLP$ent,pch=19,
     ylab="Entropy",xlab="Cosine similarity",
     cex.lab=1.5);
text(0.85,0.25,"r = -0.34",cex=1.5,col="red");

# no monos
cor.test(data_BLP$cossim[data_BLP$category!="mono"],data_BLP$ent[data_BLP$category!="mono"],method="pearson");
# t = -0.66071, df = 160, p-value = 0.5097, CI=[-0.20;0.10], est=-0.05 ()
plot(data_BLP$cossim[data_BLP$category!="mono"],data_BLP$ent[data_BLP$category!="mono"],
     pch=19,ylab="Entropy",xlab="Cosine similarity",cex.lab=1.5);
text(0.85,1,"r = -0.05",cex=1.5,col="red");


ggplot(data_BLP, aes(x=cossim,y=ent)) +
  geom_jitter(width = 0.05, height = 0, alpha = 0.3,color= "black",size=4) +
  labs(x = "cossim", y = "ent") +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"),
...
cor(data_BLP$ent,data_BLP$use_ent); # r = 0.87
text(0.25,1.75,"r = 0.87",cex=1.5);
...
ggplot(data_testing_conditions_BLP, aes(x=condition,y=score,color=Gender,group=interaction(condition,Gender))) +
  scale_color_manual(values=c(cols2[261],cols2[200])) +
  geom_hline(yintercept=0.5, linetype="dashed", 
             color = "darkgray",lwd=1.25) +
...
effect_RC7 <- as.data.frame(effect('testing_condition*RC7_hist_L2',
                            ...
                            xlevels = list(RC7_hist_L2 = -2.7:2.8,
                                           testing_condition=c('0M','1M','2M')))
...
ggplot(effect_RC7, aes(x=RC7_hist_L2,y=fit)) +
  ...
  scale_fill_manual(values=c(cols2[100],cols2[200],"#004D40FF"),
                    name="Condition",labels=c("0M","1M","2M")) +
  labs(x = "PC7: L2 History", y = 'Proportion of "yes" responses', title = 'Fitted Line - Lme4') +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"),
        axis.text = element_text(family = "CMU Serif", size = 28, color = "black"),
        text=element_text(family="CMU Serif",size=28),
...
ggplot(effect_multiexp, aes(x = multiexp, y = fit)) +
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        ...
        text=element_text(family="CMU Serif",size=28),
        legend.position=c(0.1,0.9),legend.background=element_rect(fill=NA)) +
  labs(x = 'Multilingual experience', y = 'Proportion of "yes" responses', title = 'Fitted Line - Lme4')
# More multiexp = more "yes" responses overall

plot(data_BLP_testing$yes_0M~data_BLP_testing$multiexp,pch=19);
abline(lm(data_BLP_testing$yes_0M~data_BLP_testing$multiexp), col = "red",lwd=2);
abline(h=50, lty=5);

plot(data_BLP_testing$yes_1M~data_BLP_testing$multiexp,pch=19);
abline(lm(data_BLP_testing$yes_1M~data_BLP_testing$multiexp), col = "red",lwd=2);
abline(h=50, lty=5);

plot(data_BLP_testing$yes_2M~data_BLP_testing$multiexp,pch=19);
abline(lm(data_BLP_testing$yes_2M~data_BLP_testing$multiexp), col = "red",lwd=2);
abline(h=50, lty=5);
...
# "YES" - script
ggplot(data_testing_conditions_BLP, aes(x=condition,y=score,color=script,group=interaction(condition,script))) +
  scale_color_manual(values=c(cols2[400],cols2[200])) +
  geom_hline(yintercept=0.5, linetype="dashed", 
             color = "darkgray",lwd=1.25) +
  geom_boxplot(alpha = 0.1, width=0.75, lwd=1.5, show.legend=FALSE) +
  geom_point(position = position_dodge(width=0.75),size=4) +
  labs(x = "Script", y = 'Proportion of "yes" responses') +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"),
        axis.text = element_text(family = "CMU Serif", size = 28, color = "black"),
        text=element_text(family="CMU Serif",size=28),
...
plot(data_BLP_testing$L1_L2_diff,data_BLP_testing$yes_2M,pch=19,
     xlab="Bilingual balance (L1-L2 difference)",ylab='Percent of 2M "yes" responses',cex.lab=2,cex.axis=1.5);
...
ggplot(data_BLP_testing, aes(x=L1_L2_diff,y=yes_2M)) +
  geom_hline(yintercept=50, linetype="dashed", 
             color = "darkgray",lwd=1.25) +
  geom_point(size=4) +
  stat_smooth(aes(y=yes_2M), method = lm,colour="red",fill="red",linewidth=2) +
  labs(x = "Bilingual balance (L1-L2 difference)", y = 'Percent of 2M "yes" responses') +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"),
        axis.text = element_text(family = "CMU Serif", size = 28, color = "black"),
        text=element_text(family="CMU Serif",size=28),
        legend.position=c(0.15,0.2),legend.background=element_rect(fill=NA)) +
  scale_y_continuous(limits = c(0, 100))

# 2M - cossim
cossim_2M <- subset(data_BLP_testing[data_BLP_testing$testing_condition=='2M',],select=c(sbj_ID,score_2M,cossim));
cossim_2M <- distinct(cossim_2M);
plot(cossim_2M$cossim,cossim_2M$score_2M,pch=19);
abline(lm(cossim_2M$score_2M~cossim_2M$cossim), col = "red",lwd=2);
abline(h=0.5, lty=5);
...
# plotting 2M*cossim WITH outlier
ggplot(data_BLP_testing, aes(x=cossim,y=score_2M)) +
  geom_hline(yintercept=0.5, linetype="dashed", 
             color = "darkgray",lwd=1.25) +
  geom_point(size=4) +
  stat_smooth(aes(y=score_2M), method = lm,colour="red",fill="red",linewidth=2) +
  annotate("text", x=0.85, y=0.55, label= "p=0.033",size=8,family="CMU Serif",color="red",fontface=2) +
  labs(x = "Multilingual balance (cosine similarity)", y = '2M score') +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"),
        axis.text = element_text(family = "CMU Serif", size = 28, color = "black"),
        text=element_text(family="CMU Serif",size=28),
        legend.position=c(0.15,0.2),legend.background=element_rect(fill=NA)) +
  scale_y_continuous(limits = c(0.20, 0.70))

# plotting 2M*cossim WITHOUT outlier
ggplot(data_BLP_testing[data_BLP_testing$score_2M>0.25,], aes(x=cossim,y=score_2M)) +
  geom_hline(yintercept=0.5, linetype="dashed", 
             color = "darkgray",lwd=1.25) +
  geom_point(size=4) +
  stat_smooth(aes(y=score_2M), method = lm,colour="red",fill="red",linewidth=2) +
  annotate("text", x=0.85, y=0.55, label= "p=0.057",size=8,family="CMU Serif",color="red",fontface=2) +
  labs(x = "Multilingual balance (cosine similarity)", y = '2M score') +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"),
        axis.text = element_text(family = "CMU Serif", size = 28, color = "black"),
...
ggplot(data_BLP_testing, aes(x=ent,y=score_2M)) +
  ...
  geom_point(size=4) +
  stat_smooth(aes(y=score_2M), method = lm,colour="red",fill="red",linewidth=2) +
  labs(x = "Multilingual balance (entropy)", y = '2M score') +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"),
        axis.text = element_text(family = "CMU Serif", size = 28, color = "black"),
        text=element_text(family="CMU Serif",size=28),
...
# 2M - cossim vs entropy
ggplot(data_BLP_testing, aes(x=cossim,y=ent,colour=score_2M)) +
  geom_point(size=4) +
  labs(x = "Multilingual balance (cosine similarity)", y = "Multilingual balance (entropy)") +
  scale_color_gradient2(low="blue",mid="violet",high="red",midpoint=0.5) +
  theme(panel.grid.major=element_blank(), panel.grid.minor = element_blank(),
        panel.background=element_blank(),axis.line=element_line(colour="black"),
        axis.text=element_text(family="CMU Serif",size=20,color="black"),
        text=element_text(family="CMU Serif",size=20),
        legend.position=c(0.15,0.2),legend.background=element_rect(fill=NA)) +
  labs(colour = "2M score");

# 2M - script
plot(data_BLP_testing$script,data_BLP_testing$score_2M,pch=19,xaxt="n",
     xlab="Script type",ylab='2M accuracy',cex.lab=2);
axis(1,at=c(1,2),cex.axis=1.5,
...
# 2M - category
plot(data_BLP_testing$category,data_BLP_testing$score_2M,pch=19,xaxt="n",
     xlab="Multilingual category",ylab='2M accuracy',cex.lab=2);
axis(1,at=c(1,2,3,4),cex.axis=1.5,
     labels=c("Monolingual","Bilingual","Trilingual","Quadrilingual"));
abline(h=0.5, lty=5);
...
plot(data_BLP_familiarity$category,data_BLP_familiarity$familiarity_mean,pch=19,
     xaxt="n",xlab="Multilingual category",ylab='Familiarity score',cex.lab=2,
     cex.axis=1.5);
axis(1,at=c(1,2,3,4),cex.axis=1.5,
     labels=c("Monolingual","Bilingual","Trilingual","Quadrilingual"));
abline(h=0.5, lty=5);
...
# familiarity - RC9
plot(data_BLP_familiarity$RC9_use_L4,data_BLP_familiarity$familiarity_mean,pch=19,
     xlab="RC9: L4 Use",ylab='Familiarity score',cex.lab=2,cex.axis=1.5);
abline(h=0.5, lty=5);
abline(lm(data_BLP_familiarity$familiarity_mean~data_BLP_familiarity$RC9_use_L4), col = "red",lwd=2);

# familiarity - ent
plot(data_BLP_familiarity$ent,data_BLP_familiarity$familiarity_mean,pch=19,
     xlab="Multilingual balance (entropy)",ylab='Familiarity score',cex.lab=2,cex.axis=1.5);
abline(h=0.5, lty=5);
abline(lm(data_BLP_familiarity$familiarity_mean~data_BLP_familiarity$ent), col = "red",lwd=2);
